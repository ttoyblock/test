...
func GetFunctionName(pc interface{}) string {
	return runtime.FuncForPC(reflect.ValueOf(pc).Pointer()).Name()
}
...
func GetCallInArgs(funcValue reflect.Value, funcArgs []string, inStart int) ([]reflect.Value, error) {
	...
	for i := inStart; i < funcValue.Type().NumIn(); i++ {
		...
		// yjson to go value
		err := json.Unmarshal([]byte(funcArgs[i-inStart]), inValue.Interface())
